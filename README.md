...
针对部分网络环境受限、无法正常访问目标网站的问题，本项目**内置了全协议代理分流功能**。脚本在运行时会根据配置自动下载 Xray 或 Hysteria 核心并搭建本地轻量代理，无需额外安装任何 Actions 插件。
...
- **智能自动扣费**：智能检测账户与所有处 `unpaid`（未支付）状态的订单，并自动使用用户可用余额完成支付。
...
- **内置原生代理**：集成 Xray 核心与 Hysteria 核心，完美支持 `Vless` / `Vmess` / `Trojan` / `Shadowsocks` / `Hysteria2` / `Socks5` 等主流协议节点的自动解析与本地分流环境搭建。
...
| 协议 | 链接格式 | 核心来源 |
| :--- | :--- | :--- |
| **VLESS** | `vless://...`（支持 Reality、TLS、WS、gRPC 等主流格式） | [XTLS/Xray-core](https://github.com/XTLS/Xray-core) |
| **VMess** | `vmess://...`（标准的 Base64 编码格式链接） | [XTLS/Xray-core](https://github.com/XTLS/Xray-core) |
| **Trojan** | `trojan://...` | [XTLS/Xray-core](https://github.com/XTLS/Xray-core) |
| **Shadowsocks** | `ss://...` | [XTLS/Xray-core](https://github.com/XTLS/Xray-core) |
| **Hysteria2** | `hy2://...` 或 `hysteria2://...` | [apernet/hysteria](https://github.com/apernet/hysteria) |
...
- **配置了 PROXY_NODE 节点却连不上？**
  工作流在启动 Xray/Hysteria 后会自动对 https://api.ipify.org 发起 5 次连接测试，若提示连接失败，请检查你的节点链接是否能够正常握手，或更换其他节点   尝试。
- **API 请求返回默认账号名（Account_1）？**
  这是 HidenCloud 官方 API 的已知行为，脚本会自动检测并回退到 HTML 页面提取真实邮箱，无需手动干预。
  
- **支持 Hysteria2 协议吗？**
  支持。使用 hy2:// 或 hysteria2:// 开头的链接即可，脚本会自动下载 apernet/hysteria 官方核心，无需额外配置。
